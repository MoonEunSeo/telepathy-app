...
| [S10](s10-cache-headers/README.md) | 정적 자산 재방문 왕복 | fetch + Resource Timing | 캐시 헤더 | ✅ **적용 완료 → 왕복 8회 → 1회**<br>`/assets` 는 해시 파일명이라 1년 + `immutable` |
| [S11](s11-code-splitting/README.md) | 단일 JS 번들 547.8 kB | curl + Lighthouse (폰트 차단 5회) | 라우트 코드 스플리팅 | ✅ **적용 완료 → 초기 JS 171 kB → 111 kB (−34.8%)**<br>LCP 2,412 → 2,116 ms (−12.3%) |
| [S12](s12-edge-cache/README.md) | 정적 자산이 엣지에 안 걸림 | curl / Render 문서 | Cloudflare 엣지 캐시 | ❌ **개선 못 함 → 무료 플랜에 기능이 없다**<br>`Vary: Origin` 을 원인으로 오진했다 |
| [S13](s13-self-host-fonts/README.md) | 외부 폰트 오리진 2곳 | Lighthouse (5회) | 웹폰트 자체 호스팅 | ✅ **적용 완료 → LCP 3,442 → 2,266 ms (−34.2%)**<br>Speed Index −57.4%, 점수 편차 15점 → 2점 |
...
[최적화 백로그](optimization-backlog.md)(O1~O16)에 있다.
...
- **S10** 페이지 로드로 재방문 왕복을 세니 `max-age=0` 인데도 CSS 만 요청이 없었다. Chrome
  렌더러 메모리 캐시가 재검증을 건너뛴 것이라 새 탭으로도 재현됐다. `fetch()` 로 HTTP 캐시를
  직접 태워 다시 쟀다
- **S11** Lighthouse 3회 중앙값이 Before 94 · After 82 로 나와 개선이 악화로 읽혔다. 회차가
  두 덩어리로 갈리는데 어느 쪽이 많이 나왔느냐가 중앙값을 정한 것이었다. 원인은 외부 폰트
  응답 편차(470 ms)이며 우리 변경폭(299 ms)보다 컸다. 폰트 오리진을 차단하고 5회씩 재니
  편차가 1~2 ms 로 떨어졌다
- **S12** 엣지 캐시가 안 걸리는 원인을 `Vary: Origin` 으로 단정하고 고쳤는데 배포 후에도
  `DYNAMIC` 이었다. 진짜 원인은 **Render 무료 플랜에 엣지 캐시 기능이 없다**는 것이었다.
  헤더가 그럴듯한 설명을 제공할 때가 가장 위험하다 — **고치기 전에 그 기능이 이 환경에서
  제공되는지부터 확인한다**
- **S13** 브라우저 패인에서 확인하니 woff2 요청이 0건이라 폰트가 안 붙은 것으로 보였다.
  `paint` 항목이 비어 있었다 — 패인이 프레임을 그리지 않아 폰트 요청이 발생하지 않은
  것이고, `document.fonts.size` 는 196 으로 정상이었다. **렌더에 의존하는 지표는 렌더가
  일어나는 환경(Lighthouse headless)에서 잰다**
- **S13** self-host 의 근거를 `preload` 로 잡았는데, 측정해보니 폰트가 이미 첫 페인트 전에
  도착해 preload 할 여지가 없었다. **개선안을 세운 근거가 개선 후에도 유효한지 다시 잰다**

> 브라우저 캐시를 재는 측정은 브라우저 캐시에 오염된다. S9·S10 이 같은 함정에 걸렸다.
> **캐시가 개입할 수 있는 지표는 캐시와 무관한 경로로 다시 확인한다.**
>
> 외부 오리진에 의존하는 지표도 마찬가지다. S11 에서는 폰트 CDN 이 번들 변화보다 크게
> 흔들었다. **재려는 것 하나만 남기고 나머지 변수를 차단한 뒤, 절대값이 아니라 차이를 본다.**
...
> S10 은 변경 전 코드를 5001 포트에 따로 띄워 같은 브라우저에서 대조했다 —
> 포트가 다르면 오리진이 달라 HTTP 캐시가 분리되므로 두 상태가 서로를 오염시키지 않는다.
